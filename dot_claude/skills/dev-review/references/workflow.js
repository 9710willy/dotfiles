...
const FINDING = {
  type: 'object',
  required: ['severity', 'file', 'line', 'code', 'principle', 'finding', 'suggestion', 'verified'],
  properties: {
...
const SYNTH_SCHEMA = {
  ...
  properties: {
    findings: {
      ...
      items: {
        type: 'object',
        required: ['severity', 'file', 'line', 'lenses', 'code', 'principle', 'finding', 'suggestion', 'unverified'],
        properties: {
          severity: { type: 'string', enum: ['blocker', 'major', 'minor', 'nit'] },
          file: { type: 'string' },
          line: { type: 'integer' },
...
4. Everything else passes through unchanged — same severity, code quote, principle, finding, suggestion. Set "unverified" true and copy the reviewer's note into "whatToCheck" when its verified field starts with UNVERIFIED. Do not add findings, soften severities, or pad.
...
synth.findings.sort((a, b) => (rank[a.severity] - rank[b.severity]) || a.file.localeCompare(b.file) || (a.line - b.line))
